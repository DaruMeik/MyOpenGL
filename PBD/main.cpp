...
#pragma region va->vb->ib->shader
	...
	// Shader
	Shader baseShader{ "Resource/Shader/basic.shader" };
	baseShader.Bind();
	baseShader.SetUniformMat4f("projection", projection);
	baseShader.SetUniformMat4f("view", view);
	baseShader.SetUniformMat4f("model", model);
	baseShader.SetUniform3f("iResolution", WIDTH, HEIGHT, 1.0f);
	baseShader.Unbind();
