...
float map(vec3 p){
    ...
    distToSDF = min(distToSDF, sdPlane(p, vec3(0.0f, 1.0f, 0.0f), 0.0f));
    
    float displacement = sin(5.0f * p.x + iTime*2f) * sin(10.0 * p.y + iTime*2f) * sin(15.0 * p.z + iTime*2f) * 0.1f;
    float distToSphere1 = sdSphere(p - vec3(0.0f, sin(iTime) * 1f, 0.0f), 0.5f) + displacement;
...
void main()
{
    ...
    // Camera
    vec3 camPos = vec3(4.0f*sin(iTime), 3.0f, 4.0f*cos(iTime));
    ...
    // Ray origin and Ray direction (othrographic)
    vec3 ro = camPos + uv.x * camU * camSize + uv.y * camV * camSize;
    vec3 rd = camW;
